...
class TestTail(QsvTestBase):
    ...
    def test_tail_default_count(self):
        """Test tail with default count (last 10 rows)"""
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail - show")
        ...
        for col in ["2023-01-01 14:00:00"]:
            self.assertNotIn(col, result.stdout.strip())
    
    def test_tail_specific_count(self):
        """Test tail with specific count"""
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail 3 - show")
        self.assertEqual(result.stdout.strip(), '\n'.join([
            "datetime,col1,col2,col3,str",
            "2023-01-01 21:00:00,64,65,66,Victor",
            "2023-01-01 22:00:00,67,68,69,Whiskey",
        ...
        for col in ["2023-01-01 20:00:00"]:
            self.assertNotIn(col, result.stdout.strip())
    
    def test_tail_specific_count_with_shortname_option(self):
        """Test tail with specific count and shortname option"""
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail -n 3 - show")
        self.assertEqual(result.stdout.strip(), '\n'.join([
            "datetime,col1,col2,col3,str",
            "2023-01-01 21:00:00,64,65,66,Victor",
            "2023-01-01 22:00:00,67,68,69,Whiskey",
    ...
    def test_tail_specific_count_with_longname_option(self):
        """Test tail with specific count and longname option"""
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail --number 3 - show")
        self.assertEqual(result.stdout.strip(), '\n'.join([
            "datetime,col1,col2,col3,str",
            "2023-01-01 21:00:00,64,65,66,Victor",
            "2023-01-01 22:00:00,67,68,69,Whiskey",
    ...
    def test_tail_zero_rows(self):
        ...
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail 0 - show")
        self.assertEqual(result.stdout.strip(), '\n'.join([
            "datetime,col1,col2,col3,str",
        ]))
    ...
    def test_tail_more_than_available(self):
        ...
        result = self.run_qsv_command(f"load {self.get_fixture_path('simple_manyrows.csv')} - tail 100 - show")
        self.assertEqual(result.stdout.strip(), '\n'.join([
            "datetime,col1,col2,col3,str",
