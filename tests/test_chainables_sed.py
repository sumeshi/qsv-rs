...
class TestSed(QsvTestBase):
    ...
    def test_sed_regex(self):
        ...
        # The sed command with '\d' regex should replace digits in col1 with X.
        output = self.run_qsv_command("load sample/simple.csv - sed col1 '\\d' X - show")
        
        # Check that the regex pattern correctly replaced digits with X.
        self.assert_output_contains(output, "col1,col2,col3")
        self.assert_output_contains(output, "X,2,3") # col1's '1' should be 'X'
        self.assert_output_contains(output, "X,5,6") # col1's '4' should be 'X'
        self.assert_output_contains(output, "X,8,9") # col1's '7' should be 'X'
