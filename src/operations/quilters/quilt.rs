...
use polars::prelude::{LazyFrame, col, JoinType};
...
use crate::operations::finalizers::{headers as headers_op, stats as stats_op, showquery as showquery_op, show as show_op, showtable as showtable_op, dump as dump_op};
...
fn get_usize_from_value(val: &Value, key: &str) -> Option<usize> {
    val.get(key).and_then(|v| v.as_u64().and_then(|u| usize::try_from(u).ok()))
}

pub fn quilt(controller: &mut DataFrameController, config_path_str: &str, cli_input_files: Option<Vec<PathBuf>>, output_path_str: Option<&str>, title_override: Option<&str>) {
    ...
    for (stage_name_val, stage_config_val) in &quilt_config.stages {
        let stage_name = stage_name_val.as_str().unwrap_or("unknown_stage").to_string();
        let stage_config: StageConfig = match serde_yaml::from_value(stage_config_val.clone()) {
            Ok(sc) => sc,
            Err(e) => {
                LogController::error(&format!("Error parsing config for stage '{}': {}. Skipping.", stage_name, e));
                continue;
            }
        };

        LogController::debug(&format!("Processing stage: {} (type: {})", stage_name, stage_config.stage_type));
        ...
        if let Some(source_name) = &stage_config.source {
            if let Some(df) = stage_results.get(source_name) {
                ...
                LogController::debug(&format!("Stage '{}' is using data from source stage '{}'", stage_name, source_name));
                    } else {
                LogController::error(&format!("Source stage '{}' not found for stage '{}'. Skipping stage.", source_name, stage_name));
                continue;
            }
        }
        ...
        if stage_config.stage_type == "process" {
            if let Some(steps) = &stage_config.steps {
                for (command_name_val, command_args_val) in steps {
                    ...
                    LogController::debug(&format!("Applying step: {} to stage '{}'", command_name, stage_name));
                    ...
                    match command_name {
                        "load" => {
                            ...
                            if let Some(file_str) = file_to_load_str {
                                ...
                                let path_to_load = if source_path.is_absolute() {
                                    ...
                                    config_path.parent().unwrap_or_else(|| Path::new(".")).join(source_path)
                                };
                                ...
                                if stage_output_df.is_none() && !cli_files.is_empty() {
                                     LogController::debug(&format!("Loading data from CLI for stage '{}': {:?}", stage_name, cli_files));
                                ...
                                let default_data_path = config_path.parent().unwrap_or_else(|| Path::new(".")).join("../sample/simple.csv");
                                if default_data_path.exists() {
                                     loaded_df = Some(load_op::load(&[default_data_path], ",", false));
                                }
                            }
                            ...
                            if let Some(ref df) = stage_output_df {
                                if let Some(colnames_str) = get_string_from_value(command_args_val, "colnames") {
                                    let colnames: Vec<String> = colnames_str.split(',').map(|s| s.trim().to_string()).collect();
                                    stage_output_df = Some(select::select(df, &colnames));
                                } else if let Some(colnames_vec) = get_string_vec_from_value(command_args_val, "colnames") {
                        ...
                        "head" => {
                            if let Some(ref df) = stage_output_df {
                                if let Some(num) = get_usize_from_value(command_args_val, "number").or_else(|| command_args_val.as_u64().and_then(|u| usize::try_from(u).ok())) {
                        ...
                        "changetz" => {
                             if let Some(ref df) = stage_output_df {
                                if let (Some(colname), Some(tz_from), Some(tz_to)) = (
                                    ...
                                    let dt_format = get_string_from_value(command_args_val, "dt_format");
                                    let ambiguous = get_string_from_value(command_args_val, "ambiguous");
                                    stage_output_df = Some(changetz::changetz(df, &colname, &tz_from, &tz_to, dt_format.as_deref().unwrap_or("auto"), ambiguous.as_deref().unwrap_or("earliest")));
                                } else {
                                    LogController::warn("Changetz step missing 'colname', 'tz_from', or 'tz_to'.");
                                }
                            }
                        },
                        "renamecol" => {
                            if let Some(ref df) = stage_output_df {
                                if let (Some(old_name), Some(new_name)) = (
                                    get_string_from_value(command_args_val, "from").or_else(|| get_string_from_value(command_args_val, "old_name")),
                                    get_string_from_value(command_args_val, "to").or_else(|| get_string_from_value(command_args_val, "new_name"))
                                ) {
                                    stage_output_df = Some(renamecol::renamecol(df, &old_name, &new_name));
                                } else {
                        ...
                        "uniq" => {
                            if let Some(ref df) = stage_output_df {
                                let colnames = get_string_from_value(command_args_val, "colnames")
                                    ...
                                    .or_else(|| get_string_vec_from_value(command_args_val, "colnames"));
                                stage_output_df = Some(uniq::uniq(df, colnames.as_deref()));
                            }
                        },
                        "grep" => {
                             if let Some(ref df) = stage_output_df {
                                if let Some(pattern) = get_string_from_value(command_args_val, "pattern") {
                                    let ignorecase = get_bool_from_value(command_args_val, "ignorecase");
                                    let inverted = get_bool_from_value(command_args_val, "invert_match");
                                    stage_output_df = Some(grep::grep(df, &pattern, ignorecase, inverted));
                                } else {
                        ...
                        "contains" => {
                            if let Some(ref df) = stage_output_df {
                                if let (Some(colname), Some(pattern)) = (
                                    ...
                                    let ignorecase = get_bool_from_value(command_args_val, "ignorecase");
                                    stage_output_df = Some(contains::contains(df, &colname, &pattern, ignorecase));
                                } else {
                                    LogController::warn("Contains step missing 'colname' or 'pattern'.");
                                }
                            }
                        },
                         "sed" => {
                            if let Some(ref df) = stage_output_df {
                                if let (Some(colname), Some(pattern), Some(replacement)) = (
                                    ...
                                    let ignorecase = get_bool_from_value(command_args_val, "ignorecase");
                                    stage_output_df = Some(sed::sed(df, &colname, &pattern, &replacement, ignorecase));
                                } else {
                                    LogController::warn("Sed step missing 'colname', 'pattern', or 'replacement'.");
                                }
                            }
                        },
                        "showtable" => {
                            ...
                            } else { LogController::warn("No DataFrame to showtable for stage.");}
                        },
                        "show" => {
                            if let Some(df) = &stage_output_df {
                                show_op::show(df);
                            } else { LogController::warn("No DataFrame to show for stage.");}
                        },
                        "headers" => {
                            if let Some(df) = &stage_output_df {
                                 let plain = get_bool_from_value(command_args_val, "plain");
                                 headers_op::headers(df, plain);
                            } else { LogController::warn("No DataFrame for headers for stage.");}
                        },
                        "stats" => {
                            if let Some(df) = &stage_output_df {
                                stats_op::stats(df);
                            } else { LogController::warn("No DataFrame for stats for stage.");}
                        },
                        "showquery" => {
                            if let Some(df) = &stage_output_df {
                                showquery_op::showquery(df);
                            } else { LogController::warn("No DataFrame for showquery for stage.");}
                        },
                        "dump" => {
                             if let Some(df) = &stage_output_df {
                                let path_from_yaml = get_string_from_value(command_args_val, "path")
                                    .or_else(|| get_string_from_value(command_args_val, "output"));
                                
                                let separator_char = get_string_from_value(command_args_val, "separator")
                                ...
                                if let Some(p) = path_from_yaml {
                                     let dump_path_resolved = if Path::new(&p).is_absolute() {
                                        ...
                                        config_path.parent().unwrap_or_else(|| Path::new(".")).join(p)
                                    };
                                    dump_op::dump(df, dump_path_resolved.to_str().unwrap_or_default(), separator_char.unwrap_or(','));
                                } else {
                                    LogController::warn("Dump step in YAML missing 'path' or 'output'. Will not dump from this step.");
                                }
                            } else { LogController::warn("No DataFrame to dump for stage.");}
                        },
                        _ => {
                 ...
                 LogController::warn(&format!("Stage '{}' is of type 'process' but has no steps defined.", stage_name));
            }
        } else if stage_config.stage_type == "concat" {
            LogController::warn(&format!("Stage type 'concat' for stage '{}' is not yet implemented.", stage_name));
            ...
            if let Some(sources_string_vec) = &stage_config.sources {
                if sources_string_vec.len() == 2 {
                    ...
                    if left_name.is_empty() || right_name.is_empty() {
                        LogController::error(&format!("Join stage '{}' has empty source names. Skipping.", stage_name));
                        continue;
                    }

                    if let (Some(left_df), Some(right_df)) = (stage_results.get(left_name), stage_results.get(right_name)) {
                        let join_params = stage_config.params.as_ref();
                        let how_str = join_params.and_then(|p| get_string_from_value(p, "how")).unwrap_or_else(|| "inner".to_string());
                        let key_col_name = join_params.and_then(|p| get_string_from_value(p, "key")).or_else(|| join_params.and_then(|p| get_string_from_value(p, "on")));

                        if key_col_name.is_none() {
                            LogController::error(&format!("Join stage '{}' missing 'key' (or 'on') parameter. Skipping.", stage_name));
                            continue;
                        }
                        ...
                        LogController::debug(&format!("Join stage '{}' completed using key '{}' and type '{}'", stage_name, key, how_str));

                    } else {
                        let mut missing_sources = Vec::new();
                        if stage_results.get(left_name).is_none() { missing_sources.push(left_name); }
                        if stage_results.get(right_name).is_none() { missing_sources.push(right_name); }
                        LogController::error(&format!("Could not find source DataFrame(s): {:?} for join stage '{}'. Skipping.", missing_sources, stage_name));
                        continue;
                    }
                } else {
                    LogController::error(&format!("Join stage '{}' must have exactly two sources. Found {}. Skipping.", stage_name, sources_string_vec.len()));
                    continue;
                }
            } else {
                LogController::error(&format!("Join stage '{}' is missing 'sources' attribute. Skipping.", stage_name));
                continue;
            }
        } else {
            LogController::warn(&format!("Unknown stage type: {} for stage '{}'", stage_config.stage_type, stage_name));
        }
        ...
        if let Some(df_to_store) = &stage_output_df {
            ...
            LogController::debug(&format!("Finished processing stage '{}'. Result stored.", stage_name));
        } else {
            LogController::warn(&format!("Stage '{}' did not produce a DataFrame.", stage_name));
        }
    }

    LogController::info(&format!("Quilt '{}' execution processing finished.", quilt_config.title));
    ...
    if let Some(path_str) = output_path_str {
        if let Some(final_df_to_dump) = last_processed_df {
            ...
            let absolute_path = if final_output_path.is_absolute() {
                ...
                std::env::current_dir().unwrap_or_else(|_| Path::new(".").to_path_buf()).join(final_output_path)
        };
        ...
        if let Some(parent) = absolute_path.parent() {
            ...
            dump_op::dump(&final_df_to_dump, absolute_path.to_str().unwrap_or(path_str), ',');
        } else {
            LogController::warn("No final DataFrame from quilt execution to save for --output CLI option.");
        }
    } else {
